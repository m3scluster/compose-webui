...
export const buildComposeYaml = (form) => {
  ...
  if (constraints.length) placement.constraints = constraints
  service.deploy = { runtime: form.runtime.trim() || undefined, placement: Object.keys(placement).length ? placement : undefined, replicas: Number(form.instances), resources: { limits: { cpus: Number(form.cpus), memory: Number(form.memory) } } }
